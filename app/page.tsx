      ...
      <main className="flex-1 min-h-screen flex flex-col overflow-hidden">
        <div id="main-scroll-container" className="flex-1 relative overflow-y-auto">
          ...
          <div className="relative z-10 flex flex-col gap-6 p-6">
            ...
            <div className="w-full min-h-screen lg:h-screen rounded-b-[32px] rounded-t-none p-8 md:p-12 relative overflow-hidden flex flex-col lg:flex-row items-center justify-between gap-8 bg-white/40 dark:bg-slate-950/40 border border-taupe/20 dark:border-slate-800/40 text-slate-800 dark:text-slate-100 shadow-xl backdrop-blur-md">
              ...
              <div className="absolute top-6 right-6 flex items-center gap-4 z-30 glass-panel shadow-md px-4 py-2 rounded-full border border-taupe/20 dark:border-slate-800/40 bg-white/70 dark:bg-slate-900/70 backdrop-blur-md">
                ...
                <button onClick={() => setIsAirGapped(!isAirGapped)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider transition-all cursor-pointer bg-white/40 dark:bg-slate-900/40 hover:bg-white/60 dark:hover:bg-slate-800/60"
              ...
              {/* Unified Left Content Box (covers 70% of the header height, connected to top screen) */}
              <div className="glass-panel flex flex-col justify-between p-8 rounded-b-3xl h-fit lg:h-[70%] min-h-[480px] w-full max-w-lg z-10 border-x border-b border-taupe/20 dark:border-slate-800/40 shadow-xl bg-white/30 dark:bg-slate-900/30 self-start -mt-8 md:-mt-12 rounded-t-none border-t-0">
                {/* Horizontal Action Box on Left Side of Header inside the unified card - positioned at the TOP with premium styling */}
                <div className="glass-panel shadow-lg p-2.5 flex gap-4 items-center rounded-2xl w-fit z-20 border border-amber-500/20 dark:border-amber-400/20 bg-gradient-to-r from-amber-500/5 to-orange-500/5 dark:from-amber-400/5 dark:to-orange-400/5 backdrop-blur-xl mb-6 hover:border-amber-500/35 transition-colors">
                  {[
                ...
                <div className="flex flex-col gap-4 text-left mt-auto">
                  ...
                  <h2 className="text-3xl md:text-5xl font-black font-display tracking-tight leading-tight text-slate-800 dark:text-slate-100">
                    Summon Incidents. <br/>
                    <span className="text-amber-500 dark:text-amber-400">Redefine Support.</span>
                  </h2>
              ...
              {/* Right Side: Dual Vanta Animations & VRM Anchor */}
              <div className="relative w-full lg:w-[50%] h-[400px] lg:h-[70%] flex items-center justify-center lg:justify-end overflow-visible z-10">
                {/* Vanta 1: Perfect Semicircle covering 100% screen height (h-screen) with width adjusted to 50vh */}
                <div className="absolute right-0 top-0 h-screen w-[50vh] overflow-hidden z-0 rounded-l-full border-l border-t border-b border-taupe/15 dark:border-slate-800/40 shadow-lg bg-transparent">
                  <VantaTrunk theme={theme} accentColor={settings.accentColor} className="absolute left-0 top-0 w-[100vh] h-full border-0 shadow-none rounded-none bg-transparent" />
                </div>

                {/* Vanta 2: covers 75vh vertically and overlaps Vanta 1 by 45% (positioned at right-[27.5vh]) */}
                <div className="absolute right-[27.5vh] top-[12.5vh] h-[75vh] aspect-square rounded-full overflow-hidden z-10 border-4 border-white/20 dark:border-slate-800/40 shadow-2xl bg-transparent flex items-center justify-center">
                  <VantaTrunk theme={theme} accentColor={settings.accentColor} className="w-full h-full border-0 shadow-none rounded-full bg-transparent" />
                  {/* Mascot Anchor overlaying the Vanta canvas centered */}
                  <div 
                    id="header-mascot-container" 
                    className="absolute inset-0 m-auto w-48 h-48 pointer-events-none z-20" 
                  />
                </div>
              </div>
            </div>
            ...
            {settings.mascotVisible && (
              <motion.div 
                ...
                className="w-[240px] shrink-0 flex flex-col gap-6 sticky top-20 self-start">

              {/* Character */}
              <div className="w-full h-44 flex items-center justify-center relative pointer-events-none">
                <div 
                  id="sidebar-mascot-container"
                  className="relative z-10 w-32 h-32 flex items-center justify-center pointer-events-none"
                />
              </div>
            ...
            <motion.div 
              ...
              {/* Tab bar */}
              <div className="flex-1 flex flex-col min-h-0 glass-panel border-taupe/40 rounded-2xl overflow-hidden h-full">
